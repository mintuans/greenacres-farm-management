...
const PlantsWarehouse: React.FC = () => {
    ...
    const units = ['Cây', 'Chậu', 'Giỏ', 'Bụi', 'Khóm'];
    ...
    const loadItems = async () => {
        try {
            setLoading(true);
            const data = await getWarehouseItems('plants', searchTerm);
            setItems(data);
        } catch (error) {
            console.error('Error loading plants items:', error);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleOpenModal = async (item?: WarehouseItem) => {
        if (item) {
            ...
            setEditingItem(null);
            const nextCode = await getNextWarehouseCode('plants');
            setFormData({
                item_code: nextCode,
                item_name: '',
                category: '',
    ...
    const handleOpenMediaPicker = async () => {
        ...
        try {
            const response = await getMediaFiles({ category: 'plants' });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editingItem) {
                await updateWarehouseItem('plants', editingItem.id, formData);
            } else {
                await createWarehouseItem('plants', formData);
            }
            setShowModal(false);
            loadItems();
        } catch (error) {
            console.error('Error saving item:', error);
            alert('Có lỗi xảy ra khi lưu hoa kiểng');
        }
    };

    const handleDelete = async (id: string) => {
        if (window.confirm('Bạn có chắc muốn xóa cây kiểng này?')) {
            try {
                await deleteWarehouseItem('plants', id);
                loadItems();
            } catch (error) {
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        <thead className="bg-slate-50 text-xs font-bold text-slate-500 border-b border-slate-200">
                            <tr>
                                <th className="px-6 py-4">Cây kiểng</th>
                        ...
                        <tbody className="divide-y divide-slate-100 text-sm">
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan={7} className="px-6 py-10 text-center text-slate-300 italic">Không tìm thấy cây kiểng nào</td>
                                </tr>
                            ) : (
                                items.map((item) => (
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-300">
                        <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between">
                            <div>
                                <h2 className="text-xl font-black text-slate-900">{editingItem ? 'Cập nhật cây kiểng' : 'Thêm cây kiểng mới'}</h2>
                                <p className="text-slate-400 text-[11px] font-bold uppercase tracking-widest mt-1">Kho Hoa kiểng</p>
                            </div>
                        ...
                        <form onSubmit={handleSubmit} className="p-8 overflow-y-auto max-h-[calc(90vh-160px)] custom-scrollbar">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Mã cây</label>
                                    <input
                                        required
                                        type="text"
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Tên cây</label>
                                    <input
                                        required
                                        type="text"
                                        ...
                                        placeholder="Tên cây kiểng..."
                                    />
                                </div>

                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Vị trí vườn</label>
                                    <input
                                        type="text"
                                        value={formData.location}
                                        onChange={e => setFormData({ ...formData, location: e.target.value })}
                                        className="w-full bg-slate-50 border-none rounded-2xl px-6 py-4 focus:ring-4 focus:ring-[#13ec49]/20 outline-none font-bold text-slate-900 transition-all"
                                        placeholder="Ví dụ: Vườn A, Khu B"
                                    />
                                </div>
                                <div className="space-y-2">
                            ...
                            >
                                ...
                                {editingItem ? 'Cập nhật cây kiểng' : 'Lưu cây kiểng mới'}
                            </button>
                        </form>
                    </div>
                </div>
                    ...
                    <div className="bg-white rounded-[40px] w-full max-w-4xl h-[85vh] overflow-hidden flex flex-col shadow-2xl animate-in fade-in slide-in-from-bottom-8 duration-500">
                        <div className="p-8 border-b border-slate-100 flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest mt-1">Chọn ảnh cho hoa kiểng</p>
                            </div>
