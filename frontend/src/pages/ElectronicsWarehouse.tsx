...
const ElectronicsWarehouse: React.FC = () => {
    ...
    const units = ['Máy', 'Cái', 'Bộ', 'Hệ thống', 'Thiết bị'];
    ...
    const loadItems = async () => {
        try {
            setLoading(true);
            const data = await getWarehouseItems('electronics', searchTerm);
            setItems(data);
        } catch (error) {
            console.error('Error loading electronics items:', error);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleOpenModal = async (item?: WarehouseItem) => {
        if (item) {
            ...
            setEditingItem(null);
            const nextCode = await getNextWarehouseCode('electronics');
            setFormData({
                item_code: nextCode,
                item_name: '',
                category: '',
    ...
    const handleOpenMediaPicker = async () => {
        ...
        try {
            const response = await getMediaFiles({ category: 'electronics' });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editingItem) {
                await updateWarehouseItem('electronics', editingItem.id, formData);
            } else {
                await createWarehouseItem('electronics', formData);
            }
            setShowModal(false);
            loadItems();
        } catch (error) {
    ...
    const handleDelete = async (id: string) => {
        if (window.confirm('Bạn có chắc muốn xóa thiết bị này?')) {
            try {
                await deleteWarehouseItem('electronics', id);
                loadItems();
            } catch (error) {
                    ...
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-slate-100 text-sm">
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan={7} className="px-6 py-10 text-center text-slate-300 italic">Không tìm thấy thiết bị nào</td>
                                </tr>
                            ) : (
                                items.map((item) => (
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-300">
                        <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between">
                            <div>
                                <h2 className="text-xl font-black text-slate-900">{editingItem ? 'Cập nhật thiết bị' : 'Thêm thiết bị mới'}</h2>
                                <p className="text-slate-400 text-[11px] font-bold uppercase tracking-widest mt-1">Kho Điện tử</p>
                            </div>
                        ...
                        <form onSubmit={handleSubmit} className="p-8 overflow-y-auto max-h-[calc(90vh-160px)] custom-scrollbar">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Mã thiết bị</label>
                                    <input
                                        required
                                        type="text"
                                ...
                                <div className="space-y-2">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-slate-50 border-none rounded-2xl px-6 py-4 focus:ring-4 focus:ring-[#13ec49]/20 outline-none font-bold text-slate-900 transition-all"
                                        placeholder="Ví dụ: Kho B, Tầng 3"
                                    />
                                </div>
                                <div className="space-y-2">
                            ...
                            >
                                ...
                                {editingItem ? 'Cập nhật thiết bị' : 'Lưu thiết bị mới'}
                            </button>
                        </form>
                    </div>
                </div>
                    ...
                    <div className="bg-white rounded-[40px] w-full max-w-4xl h-[85vh] overflow-hidden flex flex-col shadow-2xl animate-in fade-in slide-in-from-bottom-8 duration-500">
                        <div className="p-8 border-b border-slate-100 flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest mt-1">Chọn ảnh cho thiết bị điện tử</p>
                            </div>
