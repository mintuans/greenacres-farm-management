...
import { showcaseForgotPassword } from '@/src/services/auth.service';
...
const ForgotPassword: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const result = await showcaseForgotPassword(email);
            if (result.success) {
            ...
            <main className="flex-1 flex items-center justify-center p-6 relative overflow-hidden">
                ...
                <div className="w-full max-w-[480px] bg-white rounded-[40px] shadow-2xl shadow-slate-200/50 border border-white p-8 md:p-12 z-10 animate-in fade-in slide-in-from-bottom-8 duration-700">
                    <div className="flex flex-col items-center mb-10">
                        <h1 className="text-[#111813] text-4xl font-black leading-tight tracking-tight mb-3">Quên mật khẩu?</h1>
                        <p className="text-[#61896b] text-base font-bold text-center leading-relaxed italic opacity-80">Nhập email của bạn để nhận hướng dẫn khôi phục</p>
                    ...
                    {!message ? (
                        <form className="space-y-6" onSubmit={handleSubmit}>
                            ...
                            <div className="space-y-2 group">
                                <label className="text-[10px] font-black text-[#61896b] uppercase tracking-[0.2em] ml-2 block transition-colors group-focus-within:text-[#13ec49]">Email của bạn</label>
                                <div className="relative">
                                    <input
                                        ...
                                        required
                                    />
                                    <span className="absolute right-5 top-1/2 -translate-y-1/2 material-symbols-outlined text-[#61896b] opacity-40 group-focus-within:opacity-100 group-focus-within:text-[#13ec49] transition-all">mail</span>
                                </div>
                            </div>
                        ...
                        <div className="text-center">
                            <Link
                                to="/login"
                                className="inline-block px-8 py-4 bg-[#f0f4f1] text-[#61896b] font-black rounded-2xl hover:bg-[#e2e8e3] transition-all"
                            >
                                QUAY LẠI ĐĂNG NHẬP
                            </Link>
                        </div>
                    ...
                    <div className="mt-12 text-center space-y-2">
                        <p className="text-[#61896b] text-sm font-bold">Quay lại trang chủ?</p>
                        <Link className="inline-block text-[#13ec49] text-base font-black uppercase tracking-widest hover:underline hover:scale-105 transition-all" to="/showcase">Trang chủ</Link>
                    </div>
                </div>
            </main>
